...
public:
   RemoteOrderState(PositionManager *l_position_manager) {
   ...
   if (serverDiff.HasDifference()) {
      ...
      for (int i = 0; i < serverDiff.OpenedCount(); i++) {
         ...
         if (!localOrders.ExistsServerTicketNo(openedInfo.server_ticket_no)) {
            if (!positionManager.CreatePosition(openedInfo.server_ticket_no, openedInfo.symbol, openedInfo.order_type)) {
               do_not_open = true;
            }
         }
         if (!do_not_open) {
