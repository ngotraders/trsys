...
namespace Trsys.Models.Tests
{
    ...
    public class UserCreateIfNotExistsCommandTests
    {
        ...
        public async Task When_creates_first_time_Then_succeeds()
        {
            ...
            var id = await mediator.Send(new UserCreateIfNotExistsCommand("name", "username", "emailAddress", "pass", "Administrator"));
        ...
        public async Task When_creates_second_time_Then_succeeds_but_nothing_happens()
        {
            ...
            var mediator = services.GetRequiredService<IMediator>();
            var id = await mediator.Send(new UserCreateIfNotExistsCommand("name", "username", "emailAddress", "pass", "Administrator"));
            Assert.AreEqual(id, await mediator.Send(new UserCreateIfNotExistsCommand("name", "username", "pass", "emailAddress", "Administrator")));
