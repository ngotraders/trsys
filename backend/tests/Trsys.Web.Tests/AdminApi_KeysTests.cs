...
namespace Trsys.Web.Tests
{
    ...
    {
        [TestMethod]
        public async Task Index_search_keys()
        {
            ...
            var res = await client.GetAsync("/api/admin/keys");
        ...
        [TestMethod]
        public async Task Index_search_keys_with_pagenation()
        {
            using var host = await TestHelper.CreateTestServerAsync();
            var server = host.GetTestServer();
            var client = server.CreateClient();
            ...
            var res = await client.GetAsync("/api/admin/keys?_start=0&_end=1");
            Assert.AreEqual(HttpStatusCode.OK, res.StatusCode);
            Assert.AreEqual(2, int.Parse(res.Headers.GetValues("X-Total-Count").First()));
            var retObj = await res.Content.ReadAsStringAsync();
            Assert.IsTrue(retObj.Contains("key1"));
            ...
            res = await client.GetAsync("/api/admin/keys?_start=1&_end=2");
            Assert.AreEqual(HttpStatusCode.OK, res.StatusCode);
            Assert.AreEqual(2, int.Parse(res.Headers.GetValues("X-Total-Count").First()));
        ...
        [TestMethod]
        public async Task GetKey_should_return_ok_if_specified_key_exists()
        {
            using var host = await TestHelper.CreateTestServerAsync();
            var server = host.GetTestServer();
            var client = server.CreateClient();
            ...
            var res = await client.GetAsync("/api/admin/keys/" + id);
            Assert.AreEqual(HttpStatusCode.OK, res.StatusCode);
            var retObj = await res.Content.ReadAsStringAsync();
            Assert.IsTrue(retObj.Contains("key1"));
        }
        ...
        [TestMethod]
        public async Task GetKey_should_return_not_found_if_specified_key_not_exists()
        {
            using var host = await TestHelper.CreateTestServerAsync();
            var server = host.GetTestServer();
            var client = server.CreateClient();
            await client.LoginAsync();
            var res = await client.GetAsync($"/api/admin/keys/{Guid.NewGuid()}");
        ...
        [TestMethod]
        public async Task PostKey_should_return_ok_given_key_type_specified()
        {
            using var host = await TestHelper.CreateTestServerAsync();
            var server = host.GetTestServer();
            var client = server.CreateClient();
