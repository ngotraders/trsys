...
public static class DatabaseInitializer
{
    ...
    public static async Task SeedDataAsync(IApplicationBuilder app)
    {
        ...
        var userManager = scope.ServiceProvider.GetRequiredService<UserManager<TrsysUser>>();
        var user = await userManager.FindByNameAsync("admin");
        var identity = new TrsysUser("admin");
        var passwordHash = userManager.PasswordHasher.HashPassword(identity, "P@ssw0rd");
        if (user != null)
        {
            await mediator.Send(new UserChangePasswordHashCommand(user.Id, passwordHash));
            return;
        }
