...
class RemoteOrderState
{
   ...
   void m_initialize()
   {
      ...
      for (int i = 0; i < arr_position_count; i++)
      {
         ...
         if (m_index_of(info.server_ticket_no) >= 0)
            continue;
...
class FailedLocalOrderList
{
   List<LocalOrderInfo> *m_copy_trade_info_list;

public:
   FailedLocalOrderList()
   {
      m_copy_trade_info_list = new List<LocalOrderInfo>();
   };
   ~FailedLocalOrderList()
   {
      delete m_copy_trade_info_list;
   };
   void Failed(LocalOrderInfo &copy_trade_info)
   {
      m_copy_trade_info_list.Add(copy_trade_info);
   };
   void Remove(LocalOrderInfo &copy_trade_info)
   {
      for (int i = 0; i < m_copy_trade_info_list.Length(); i++)
      {
         if (m_copy_trade_info_list.Get(i).local_ticket_no == copy_trade_info.local_ticket_no)
         {
            m_copy_trade_info_list.Remove(i);
         }
      };
   };
   int TicketNoFailedCount(long local_ticket_no)
   {
      int failed_count = 0;
      for (int i = 0; i < m_copy_trade_info_list.Length(); i++)
      {
         if (m_copy_trade_info_list.Get(i).local_ticket_no == local_ticket_no)
         {
            failed_count++;
         }
      };
